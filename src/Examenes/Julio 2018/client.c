...
int ds,tam,num,valor,idSem,idCola,cont,idMem;
...
if(num_dev!=-1){
	...
	m.canal = num_dev + MSQ_TYPE_BASE;
	m.d = *((struct shm_dev_reg*)(dir+num_dev*sizeof(struct shm_dev_reg)));
	msgsnd(idCola,&m,sizeof(m)-sizeof(long),0);
	semop(idSem,&sumar[num_dev],1);

}else{
	semop(idSem,restar,MAX_SEM);
        for(num_dev=0;num_dev<=4;num_dev++){
            m.canal = num_dev + MSQ_TYPE_BASE;
            m.d = *((struct shm_dev_reg*)(dir+num_dev*sizeof(struct shm_dev_reg)));
            msgsnd(idCola,&m,sizeof(m)-sizeof(long),0);
        }
	...
	while(1){
		...
		if(*cad == 'H'){
			...
			*((struct shm_dev_reg*)(dir+d.num_dev*sizeof(struct shm_dev_reg))) = d;
			...
			semop(idSem,&restar[num],1);
			d = *((struct shm_dev_reg*)(dir+num*sizeof(struct shm_dev_reg)));
			d.num_dev = num;
			d.estado = 1;
			d.contador[cont] = valor;
			*((struct shm_dev_reg*)(dir+num*sizeof(struct shm_dev_reg))) = d;
			semop(idSem,&sumar[num],1);
			cad[0] = 'O';
	                *((int*)(cad+1)) = num;
        	        sendto(ds,cad,5,0,(struct sockaddr*)&cliente,sizeof(cliente));
