...
int ds,n_cont,tam,indice,valor,pidM,num_dev,n,i,idCola,idMem,idSem;
char deskr[100],cad[100],*dir,env[100];
...
idSem = semget(CLAVE,5,066);

idCola = msgget(CLAVE,IPC_CREAT | 0666);

if(fork() != 0){
	while(msgrcv(idCola,&rec,sizeof(rec)-sizeof(long),pidM,0)>0){
		if(rec.men.num_dev != 0){
			semop(idSem,&restar[rec.men.num_dev],1);
			envi.canal = rec.men.num_dev;
			memcpy(&envi.d,dir+(rec.men.num_dev-1)*DEVOFFSET,DEVOFFSET);
			msgsnd(idCola,&envi,DEVOFFSET,0);
			semop(idSem,&sumar[rec.men.num_dev],1);
		}else{
			semop(idSem,&restar[1],4);
			for(i=1;i<5;i++){
				envi.canal = i;
	                        memcpy(&envi.d,dir+(i-1)*DEVOFFSET,DEVOFFSET);
        	                msgsnd(idCola,&envi,DEVOFFSET,0);
			}
...
while(1){
	...
	cad[n] = '\0';
	if(*cad == 'H'){
		sscanf(cad,"HLO <%d> <%d> %s",&num_dev,&n_cont,deskr);
		sprintf(env,"OK <%d>",num_dev);
		sendto(ds,env,strlen(env),0,(struct sockaddr*)&cliente,sizeof(cliente));
