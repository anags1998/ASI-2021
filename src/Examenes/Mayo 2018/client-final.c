...
int i,ds,idMem,idSem,idCola,n,tam,pidM,num_dev,n_cont,indice,valor;
struct sockaddr_in servidor,cliente;
char cad[100],*dir,descr[100],comando[10];
struct shm_dev_reg d;
...
if(fork() != 0){
	while(msgrcv(idCola,&rec,sizeof(struct recibir)-sizeof(long),pidM,0)>0){
		if(rec.m.num_dev != 0){
			semop(idSem,&restar[rec.m.num_dev],1);
			env.canal = rec.m.num_dev;
			memcpy(&env.d,dir+(rec.m.num_dev-1)*DEVOFFSET,DEVOFFSET);
			msgsnd(idCola,&env,DEVOFFSET,0);
			semop(idSem,&sumar[rec.m.num_dev],1);
		}else {
